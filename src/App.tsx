  ...
  if (logueado) {
    return (
      <Dashboard
        ...
        onSalir={() => { setLogueado(false); setUsuario(''); setPassword(''); }}
      />
    );
  }
    ...
    <div className="min-h-screen flex items-center justify-center"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold" style={{ color: '#1e3a5f' }}>Sistemas MYA</h1>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div>
            ...
            <select value={empresaSeleccionada.codigo}
              onChange={(e) => setEmpresaSeleccionada(empresas.find(emp => emp.codigo === e.target.value) || empresas[0])}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500">
              {empresas.map((emp) => (
            ...
            <p className="text-xs text-gray-400 mt-1">Cédula: {empresaSeleccionada.cedula}</p>
          </div>
